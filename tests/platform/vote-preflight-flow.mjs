...
firstSeat.pages.submission.find(m=>m.type==='form').inputFields.forEach(f=>f.required=false);
firstSeat.pages.voting.find(m=>m.type==='form').inputFields=['phone','email','instagram'].map(binding=>({id:binding,binding,type:binding==='phone'?'tel':binding==='email'?'email':'instagram',label:binding,placeholder:'',help:'',maxLength:binding==='phone'?13:binding==='email'?254:30,required:false,options:[]}));
const admin=(path,method,body,status)=>request('http://127.0.0.1:8791/api/admin/'+path,method,body,status);
let event=await admin('events','POST',{...firstSeat,privacyPolicy:'로컬 테스트용 처리방침',contactUrl:'mailto:test@example.invalid',slug:'optional-flow-'+Date.now(),title:'통합 검증 이벤트'},201);const path='events/'+event.id;
...
await request(publicUrl+'/participate','POST',{...submission,requestKey:crypto.randomUUID(),message:'',participant:{}},201);
assert.deepEqual((await admin(path+'/participant-page?page=1')).columns,[]);
...
let entries=await admin(path+'/entries');assert.equal(entries.length,2);assert.ok(!JSON.stringify(entries).includes(participant.email));
...
const check=async(p,token='mock-voter_session',status=200)=>request(publicUrl+'/vote-check','POST',{participant:p,turnstileToken:token},status);
await check(participant,'invalid',422);
assert.equal((await check(participant)).duplicate,false);
await request(publicUrl+'/participate','POST',vote,201);
for(const field of ['phone','email','instagram'])assert.equal((await check({[field]:participant[field]})).duplicate,true);
assert.equal((await check({email:participant.email.toUpperCase()})).duplicate,true);
assert.equal((await check({instagram:'@TEST_ARENA'})).duplicate,true);
assert.equal((await check({email:'different@example.invalid'})).duplicate,false);
const duplicateReceipt=await request(publicUrl+'/participate','POST',{...vote,requestKey:crypto.randomUUID()},409);
assert.equal(duplicateReceipt.code,'DUPLICATE_VOTE');
assert.equal((await request(publicUrl+'/participate','POST',vote,201)).kind,'voting');
await request(publicUrl+'/participate','POST',{...vote,requestKey:crypto.randomUUID()},409);
await request(publicUrl+'/participate','POST',{...vote,requestKey:crypto.randomUUID(),participant:{}},201);
const candidates=await admin(path+'/candidates');assert.equal(candidates[0].votes,2);
...
console.log('Protected preflight duplicate lookup and full vote flow passed: consent, encrypted submission, review, candidate confirmation, vote, duplicate rejection, result, scoped reset.');
