...
firstSeat.pages.submission.find(m=>m.type==='consent').consents=[{id:'privacy',label:'개인정보 확인',body:'테스트 개인정보 원문'},{id:'work-license',label:'응모작 활용 확인',body:'테스트 활용 원문'},{id:'extra-confirmation',label:'추가 확인',body:'추가 확인 원문'}];
const admin=(path,method,body,status)=>request('http://127.0.0.1:8791/api/admin/'+path,method,body,status);
let event=await admin('events','POST',{...firstSeat,privacyPolicy:'로컬 테스트용 처리방침',contactUrl:'mailto:test@example.invalid',slug:'flow-'+Date.now(),title:'통합 검증 이벤트'},201);const path='events/'+event.id;
for(const [stage,kind] of [['submission','privacy'],['submission','work-license'],['voting','privacy']])event=await admin(path+'/policy','POST',{stage,kind,body:'로컬 테스트 전용 동의문. 실제 운영에 사용하지 않습니다.',revision:event.revision});
...
assert.equal(view.event.pages.voting.length,0);assert.equal(view.policies.length,3);assert.ok(view.policies.some(p=>p.body.includes('추가 확인 원문')));
const participant={name:'테스트',phone:'01000000000',email:'test@example.invalid',instagram:'test_arena'};
const submission={revision:view.revision,requestKey:crypto.randomUUID(),turnstileToken:'mock-submission',message:'함께 만드는 첫 기록',participant,policyIds:view.policies.map(p=>p.id)};
const missingConsent={...submission,requestKey:crypto.randomUUID(),policyIds:submission.policyIds.slice(0,2)};await request(publicUrl+'/participate','POST',missingConsent,409);
...
const vote={revision:view.revision,requestKey:crypto.randomUUID(),turnstileToken:'mock-vote',message:'',candidateId:view.candidates[0].id,participant,policyIds:view.policies.map(p=>p.id)};
...
console.log('Local real Workers/D1 flow passed: consent, encrypted submission, review, candidate confirmation, vote, duplicate rejection, result, scoped reset.');
