...
const browser=await chromium.launch({channel:'chrome'});try{
 ...
 for(const p of [a,b]){await p.goto('http://127.0.0.1:5191/admin');await p.locator('.admin-sidebar').getByRole('button',{name:'콘텐츠 편집',exact:true}).click();await p.locator('.editor-heading').waitFor();}
