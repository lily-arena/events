...
firstSeat.pages.submission.find(m=>m.type==='consent').consents=[{id:'privacy',label:'개인정보 확인',body:''},{id:'work-license',label:'응모작 활용 확인',body:''},{id:'extra-confirmation',label:'추가 확인',body:''}];
const admin=(path,method,body,status)=>request('http://127.0.0.1:8791/api/admin/'+path,method,body,status);
let event=await admin('events','POST',{...firstSeat,privacyPolicy:'로컬 테스트용 처리방침',contactUrl:'mailto:test@example.invalid',slug:'footer-flow-'+Date.now(),title:'통합 검증 이벤트'},201);const path='events/'+event.id;
...
console.log('Footer-only consent Workers/D1 flow passed: consent, encrypted submission, review, candidate confirmation, vote, duplicate rejection, result, scoped reset.');
