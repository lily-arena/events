...
function Field({field}:{field:FormInput}){
 ...
 return <div className={`field ${field.binding==='message'?'sentence-field':''}`}><label htmlFor={id}>{field.label}{!field.required&&' (선택)'}</label>{instagram?<div className="instagram-input"><span aria-hidden="true">@</span>{control}</div>:control}
