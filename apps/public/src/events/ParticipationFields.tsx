...
import {ageOnDate,minimumAgeMessage} from '../../../../packages/domain/src/calendar-date';
...
function Field({field}:{field:FormInput}){
 ...
 const ageError=value&&field.binding==='birthDate'&&field.minAge!==undefined&&ageOnDate(value)<field.minAge?minimumAgeMessage(field.minAge):'';
 const error=ageError||(touched&&invalid&&value.trim()?formatMessage:'');
 ...
 const control=field.type==='date'?<DateWheelField id={id} name={inputName(field)} label={field.label} value={value} required={field.required} birthDate={field.binding==='birthDate'} error={error} describedBy={common['aria-describedby']} onChange={v=>{setValue(v);setTouched(true);}} onInvalid={()=>setTouched(true)}/>:field.type==='textarea'?<textarea {...common}/>:field.type==='select'?<select {...common}><option value="">{field.placeholder||'선택해주세요.'}</option>{field.options.map(option=><option key={option}>{option}</option>)}</select>:<input {...common} type={field.type==='email'?'email':field.type==='tel'?'tel':'text'} inputMode={field.binding==='phone'||field.type==='tel'?'tel':field.type==='number'?'decimal':field.type==='email'?'email':undefined} pattern={inputPattern(field)} autoCapitalize={instagram?'none':undefined} autoComplete={field.binding==='phone'?'tel-national':field.binding==='email'?'email':field.binding==='name'?'name':undefined}/>;
