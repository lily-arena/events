...
export function InputFieldsEditor({items,onChange}:{items:FormInput[];onChange:(v:FormInput[])=>void}){
 ...
 {<label className="field">필수 여부<select value={field.required?'required':'optional'} onChange={e=>patch(index,{required:e.target.value==='required'})}><option value="required">필수</option><option value="optional">선택</option></select></label>}
