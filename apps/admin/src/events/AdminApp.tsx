...
export default function AdminApp() {
 ...
 useEffect(()=>{let active=true;(async()=>{
  ...
  await Promise.all(rows.map(async row=>{const d=JSON.parse(row.draft_json) as EventDraft;const policies=await request<{stage_id:string;kind:string;body:string}[]>('events/'+row.id+'/policies');for(const stage of ['submission','voting','result'] as const){for(const module of d.pages[stage])if(module.type==='consent'&&!module.consents){module.consents=consentItems(module,stage).map(item=>{const policy=policies.find(p=>p.stage_id===stage&&p.kind===item.id);if(!policy)return item;const decoded=decodePolicy(policy.body);return {...item,body:decoded.body,label:decoded.label??item.label};});}}row.draft_json=JSON.stringify(d);}));if(active){setAccount(session.email);setLocal(session.local);setInitial(rows.map(decode));}}
