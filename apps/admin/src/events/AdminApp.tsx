...
export default function AdminApp() {
 ...
 function decode(row:Row):EventDraft {return {...JSON.parse(row.draft_json),id:row.id,visibility:row.visibility,editorRevision:row.revision};}
 ...
 return <AdminPreview storage={{initialEvents:initial,account,local,
  ...
  async publish(draft){return decode(await request<Row>('events/'+draft.id+'/save-publish','POST',{revision:draft.editorRevision,draft}));},
  async checkRevision(id){return request<{revision:number}|null>('events/'+id+'/edit-state');},
  ...
  operations: (event,section,onChanged,onDeleted) => <EventOperations onDeleted={onDeleted} section={section} event={event} onChanged={row=>{onChanged?.(decode(row));}}/>,
  async save(draft){return decode(await request<Row>('events/'+draft.id,'PUT',{revision:draft.editorRevision,draft}));},
