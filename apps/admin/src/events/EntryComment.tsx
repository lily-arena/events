...
export function EntryComment({base,id,initial,revision,onSaved}:{base:string;id:string;initial:string;revision:number;onSaved:(value:{review_comment:string;comment_revision:number})=>void}){
 ...
 return <div className="entry-comment"><div className="entry-comment-control"><input type="text" aria-label="심사 코멘트" maxLength={300} value={text} disabled={busy} onChange={e=>setText(e.target.value)} placeholder="코멘트 작성" onKeyDown={e=>{if(e.key==='Enter'&&!e.nativeEvent.isComposing){e.preventDefault();void save();}}}/><Button kind="small" disabled={busy||text===initial} onClick={save}>{busy?'저장 중':'저장'}</Button></div>{error&&<small role="alert">{error}</small>}</div>;
}
