...
export function EntryReview({base,revision,onChanged}:{base:string;revision:number;onChanged:()=>Promise<void>}){
 ...
 const [data,setData]=useState<Page|null>(null),[loading,setLoading]=useState(true),[busy,setBusy]=useState(false),[error,setError]=useState('');
 ...
 async function review(entry:Entry,next:string){setBusy(true);setError('');try{await api(base+'/review','POST',{entryId:entry.id,status:next,revision:entry.revision});await onChanged();}catch(e){setError(e instanceof Error?e.message:'상태를 변경하지 못했습니다.');}finally{setBusy(false);setReload(n=>n+1);}}
 ...
 {data?.entries.map(entry=><tr key={entry.id}><td className="entry-message">{entry.message}</td><td><time dateTime={new Date(entry.created_at).toISOString()}>{date.format(entry.created_at)}</time></td><td><ReviewStatus status={entry.status}/></td><td>{entry.locked?<span>후보 확정됨</span>:<select aria-label={`${entry.message} 상태 변경`} disabled={busy||loading} value={entry.status} onChange={e=>review(entry,e.target.value)}>{options.map(([value,label])=><option key={value} value={value}>{label}</option>)}</select>}</td></tr>)}
