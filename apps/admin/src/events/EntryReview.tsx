...
interface Entry {id:string;message:string;created_at:number;status:string;revision:number;locked:number;review_comment:string;comment_revision:number}
...
export function EntryReview({base,revision,onChanged}:{base:string;revision:number;onChanged:()=>Promise<void>}){
 ...
 <div className="admin-table-scroll" aria-busy={loading}><table className="admin-data-table"><thead><tr><th scope="col">문구</th><th scope="col">접수일시</th><th scope="col">상태</th><th scope="col">상태 변경</th><th scope="col">심사 코멘트</th></tr></thead><tbody>
 {data?.entries.map(entry=><tr key={entry.id}><td className="entry-message">{entry.message}</td><td><time dateTime={new Date(entry.created_at).toISOString()}>{date.format(entry.created_at)}</time></td><td><ReviewStatus status={entry.status}/>{pending.has(entry.id)&&<small className="row-saving">저장 중</small>}</td><td>{entry.locked?<span>후보 확정됨</span>:<select aria-label={`${entry.message} 상태 변경`} disabled={pending.has(entry.id)||loading} value={entry.status} onChange={e=>review(entry,e.target.value)}>{options.map(([value,label])=><option key={value} value={value}>{label}</option>)}</select>}</td><td><EntryComment base={base} id={entry.id} initial={entry.review_comment??''} revision={entry.comment_revision??0} onSaved={value=>setData(d=>d?{...d,entries:d.entries.map(x=>x.id===entry.id?{...x,...value}:x)}:d)}/></td></tr>)}
 {!loading&&!data?.entries.length&&<tr><td colSpan={5}>조건에 맞는 응모작이 없습니다.</td></tr>}
 </tbody></table></div>
