...
export default {
 async fetch(request: Request,env:Env):Promise<Response> {
  ...
  } catch(error) {return json({error:adminErrorMessage(error),code:error instanceof Error&&error.message.includes('EDIT_CONFLICT')?'EDIT_CONFLICT':undefined},409);}
 }
};
