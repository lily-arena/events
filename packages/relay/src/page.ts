...
export async function handlePage(request:Request,env:GatewayEnv,shellOrigin:string):Promise<Response>{
 ...
 try{
  ...
  const shell=await fetch(new URL('/app.html',shellOrigin),{redirect:'error'});if(!shell.ok)throw new Error('Page shell unavailable');
  const html=pageMetadata(await shell.text(),title,description,new URL(url.pathname,env.PUBLIC_ORIGIN).href);
