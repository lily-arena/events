...
export async function handlePage(request:Request,env:GatewayEnv,shellHtml:string):Promise<Response>{
 ...
 try{
  ...
  const html=pageMetadata(shell,title,description,new URL(url.pathname,env.PUBLIC_ORIGIN).href).replace('</body>',(admin||landing?'':`<script type="application/json" id="event-bootstrap">${bootstrap}</script>`)+'</body>');
