...
export interface ConsentItem {id:string;label:string;body:string;required?:boolean}
...
export function encodePolicy(item:ConsentItem,privacyPolicy?:string){return JSON.stringify({...(privacyPolicy!==undefined?{privacyPolicy}:{}),format:'events-consent-v1',label:item.label,body:item.body,required:item.required!==false});}
